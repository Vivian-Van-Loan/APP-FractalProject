...
int main(int argc, char* argv[]) {
    ...
    if (info.rank == 0) {
        printf("Time for all %d MPI and %s OpenMP threads to finish computing fractal of size (%zu, %zu): %f seconds\n", info.num_procs, getenv("OMP_NUM_THREADS"), info.total_grid_width, info.total_grid_height, end_time - start_time);
    }
...
void main_loop(FractalInfo info) {
    ...
    while (true) {
        if (compute) {
            //printf("Proc %d computing fractal with offset and dims: (%zu, %zu), (%zu, %zu)", info.rank, info.sub_grid_offset_x, info.sub_grid_offset_y, info.sub_grid_size_x, info.sub_grid_size_y);
...
void compute_fractal(FractalInfo* info) {
    ...
    for (size_t y_block = 0; y_block < info->sub_grid_size_y; y_block += BLOCK_SIZE) {
        ...
        if ((info->sub_grid_size_y - y_block) > info->num_procs * 4) { //stop them from fighting over each other at small remaining values
            int request;
            MPI_Status status;
            MPI_Iprobe(MPI_ANY_SOURCE, TAG_WORK_REQUEST, MPI_COMM_WORLD, &request, &status);
            if (request) {
